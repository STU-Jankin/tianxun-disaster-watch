...
test("adds a bounded and evidence-labelled population and infrastructure exposure workflow", async () => {
  ...
  for (const phrase of ["人口与承灾体暴露", "范围内模型人口", "人口指数已生成", "已映射建筑", "已映射道路", "关键设施", "OSM 筛查范围", "已切换为双尺度筛查", "计算暴露度并叠加地图", "继续查询 OSM 分块", "不是受损建筑数", "不代表里程或通行状态"]) assert.ok(dashboard.includes(phrase));
...
test("separates landslide forecast verification from event detection metrics", async () => {
  ...
  for (const text of ["滑坡预测命中率", "滑坡预测精确率", "Brier 分数", "下载滑坡模板", "tianxun.landslide-benchmark/v2", "无事件对照", "滑坡概率阈值校准", "官方历史试验库", "导入首批草稿", "单国最多2条", "天巡区域筛查（重庆/江苏）"]) assert.ok(center.includes(text));
