...
export function buildTaskExportArtifact(tasks: Record<string, unknown>[], format: TaskExportFormat, actor: string, generatedAt = new Date().toISOString()) {
  ...
  if (format === "geojson") {
    ...
    const rows = snapshots.map((task) => ({
      ...
      opportunity_start_utc: task.opportunityStart,
      opportunity_end_utc: task.opportunityEnd,
      reachable_near_ground_range_km: task.opportunityReachableNearKm,
      reachable_far_ground_range_km: task.opportunityReachableFarKm,
      scene_near_edge_ground_range_km: task.opportunitySceneNearEdgeKm,
      scene_far_edge_ground_range_km: task.opportunitySceneFarEdgeKm,
      footprint_range_margin_km: task.opportunityFootprintRangeMarginKm,
      incidence_constraint_semantics: task.opportunityIncidenceConstraintSemantics,
      reachable_look_sides: Array.isArray(task.opportunityReachableLookSides) ? task.opportunityReachableLookSides.join("|") : "",
      sensor_parameter_status: task.sensorParameterStatus,
      cyclone_tracking_target: task.cycloneTrackingTarget,
      tracking_valid_from_utc: task.trackingValidFrom,
      tracking_valid_to_utc: task.trackingValidTo,
      tracking_lead_hours: task.trackingLeadHours,
      tracking_center_latitude: task.trackingCenterLatitude,
      tracking_center_longitude: task.trackingCenterLongitude,
      tracking_center_basis: task.trackingCenterBasis,
      tracking_threshold_knots: task.trackingThresholdKnots,
