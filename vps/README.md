...
把已申请的 FIRMS key 和高德 Web 服务 Key（`AMAP_WEB_SERVICE_KEY`）放入 `engine.env`。高德 JavaScript Key 不能替代 Web 服务 Key；该值只保存在服务器端，浏览器通过 `/api/routing` 受控调用。通知器默认执行“全等级监测、红橙重点预警、黄色条件触发、蓝色静默留存”：`MIN_NOTIFY_SEVERITY=orange`，黄色仅在无锡/江苏等重点范围、优先级达到 80，或重点范围的可信权威预报需要保留灾前观测机会时例外汇报。`MIN_NOTIFY_PRIORITY` 只用于判断红橙事件的优先级跨档变化，不能单独把普通黄色事件变成告警。

首次成功采集只建立基线并发送一条汇总，不会把现有数百条事件逐条轰炸飞书。之后仅推送：

- 新的红色、橙色主事件；
- 黄色/蓝色升级为橙色、红色，以及橙色升级为红色；
- 无锡、江苏等重点范围内满足配置条件的黄色例外事件；
- 红橙事件的独立证据增加、定位质量改善、AOI 达到可规划条件；
- 红橙台风中心移动超过 150 km 或收到新一期官方路径/风圈；
- 连续三次采集失败的数据源及恢复通知。
- 权威撤销、事件身份隔离以及由此自动取消的卫星任务。

普通黄色、蓝色事件继续进入聚合和状态基线，但不会因为分数较高、证据增加或坐标改善就反复打扰值守人员。同一台风的日常报文只更新主事件；等级、路径、风圈和任务可行性没有实质变化时不重复推送。低可信红橙信号仍会送达，但消息会明确标记“待核验”，不得写成已确认灾害。
