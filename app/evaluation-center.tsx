...
import type { DetectionEvaluationReport, EvaluationBenchmarkCase, EvaluationObjective } from "../lib/evaluation-center";
...
export function EvaluationCenter({ open, role, onClose }: { open: boolean; role: "viewer" | "operator" | "admin"; onClose: () => void }) {
  ...
  const downloadLandslideTemplate = () => {
    const template = {
      schema: "tianxun.landslide-benchmark/v1",
      generatedAt: new Date().toISOString(),
      note: "仅填写来自权威公告、调查报告或核验目录的真实事件。verificationStatus 建议先使用 draft，人工复核后再改为 verified。",
      cases: [{
  ...
  return <aside className="evaluation-panel" hidden={!open} aria-hidden={!open || undefined} role="dialog" aria-modal="true" aria-label="系统评测中心">
    ...
    <div className="evaluation-body">
      <section className="evaluation-principle">
        ...
        <p>事件检测和事前预测分开统计。预测必须在灾害发生前入库、有效期覆盖发生时刻并命中核验位置；非穷举样本库不能计算误报率。</p>
      </section>
      ...
      <section className="evaluation-results">
        ...
        {!report ? <div className="evaluation-empty">录入权威样本后运行评测，系统会回查保存的历史快照。</div> : report.results.map((result) => <article key={result.caseId} className={`evaluation-result ${result.status}`}>
          ...
          {result.status === "detected" ? <dl>
            ...
            <div><dt>{result.objective === "landslide_forecast" ? "空间命中" : "位置误差"}</dt><dd>{result.objective === "landslide_forecast" ? result.spatialMatch === "geometry_contains" ? "预测面覆盖" : `点容差 · 中心距 ${distance(result.locationErrorKm)}` : distance(result.locationErrorKm!)}</dd></div>
            <div><dt>匹配产品</dt><dd>{result.matchedTitle}</dd></div>
      ...
      {role === "admin" ? <details className="evaluation-create">
        ...
        <form onSubmit={createCase}>
          ...
          {form.objective === "event_detection" ? <label>期望等级<select value={form.expectedSeverity} onChange={(event) => setForm((current) => ({ ...current, expectedSeverity: event.target.value as EvaluationForm["expectedSeverity"] }))}><option value="">不核对等级</option><option value="blue">蓝色</option><option value="yellow">黄色</option><option value="orange">橙色</option><option value="red">红色</option></select></label> : <label>最低风险值 %<input required type="number" min="80" max="100" step="1" value={form.minimumForecastRiskPercent} onChange={(event) => setForm((current) => ({ ...current, minimumForecastRiskPercent: event.target.value }))} /></label>}
    ...
    <footer>评测模型 {report?.modelVersion ?? "tianxun-evaluation-v2"} · 报告按运行时快照固化，后续样本修改不会改写历史报告。</footer>
  </aside>;
}
...
function initialForm(objective: EvaluationObjective, hazard: HazardType): EvaluationForm {
  ...
  return {
    title: "", hazard, objective, hazardSubtype: "landslide", occurredAt: localDateTime(new Date(Date.now() - 60 * 60_000)), latitude: "", longitude: "",
