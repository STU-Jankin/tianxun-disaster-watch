...
- 暴露度结果不等同于实时人口、设施存量或受损评估：WorldPop 年份、空间分辨率、查询范围、OSM 数据时点、查询时间和缓存状态会随结果保存；OSM 零记录不会被解释为现实中不存在，也不会自动降低人口暴露度指数。默认公共 Overpass 的任务总量上限为 3,500 km²，任务按不超过 750 km² 的非重叠几何分块受控续算，建筑、道路与设施均按 OSM 类型和 ID 跨块去重；标准 30 km 地震筛查圆通常拆为 4 块。配置独立的 `china_daily` 中国日更镜像后可按运维设定扩大到最高 100,000 km²，但仍禁止把全国建筑和道路作为一次全量查询。高德 POI 可补充中国境内关键设施位置，不能替代建筑、道路存量统计或道路实时通行状态。
- 中国 OSM 数据服务器的部署边界、日更切换和应用配置见 [`vps/osm-china/README.md`](vps/osm-china/README.md)。
