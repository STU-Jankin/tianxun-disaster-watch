...
- 详情页提供共享的值守研判记录：待研判、研判中、持续监测、已确认、已驳回和已结束均采用受控状态转换；每次保存使用乐观版本锁并写入不可变审计历史，避免多人值守时静默覆盖。
- 红色、橙色告警可由操作员显式确认收到，确认绑定当前告警等级版本；后续升级后必须重新确认。人工暴露度和脆弱性指数必须同时填写数据或专家依据，两项齐全后才生成独立的影响风险初筛分。
- 事件详情可按当前 AOI 查询 WorldPop 指定年份人口模型，并通过公共 OpenStreetMap Overpass 筛查已映射建筑、道路和医疗/应急/避难/教育/电力/供排水设施；AOI 与设施点会叠加到地图，结果按事件版本和 AOI 指纹持久化缓存。
- 暴露度结果不等同于实时人口、设施存量或受损评估：WorldPop 年份、空间分辨率、查询范围和 OSM 数据时点会随结果保存；OSM 零记录不会被解释为现实中不存在，也不会自动降低人口暴露度指数。
