...
- 提供 `/api/visibility` 两级仿真接口：未配置外部服务时使用当前 TLE/SGP4 生成轨道近接候选，显示最近距离、升降轨和地图搜索圈，并明确其不代表 SAR 可成像；配置 `SATELLITE_VISIBILITY_API_URL` 后才升级为传感器级窗口。修改 AOI、时间窗、载荷或轨向约束会自动作废旧机会，轨道级粗筛结果不得直接排程或下发。
