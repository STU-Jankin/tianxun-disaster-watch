...
- 暴露度结果不等同于实时人口、设施存量或受损评估：WorldPop 年份、空间分辨率、查询范围、OSM 数据时点、查询时间和缓存状态会随结果保存；OSM 零记录不会被解释为现实中不存在，也不会自动降低人口暴露度指数。默认公共 Overpass 只执行不超过 3,500 km² 的保守筛查，能够覆盖系统标准 30 km 地震筛查圆；配置独立的 `china_daily` 中国日更镜像后可按运维设定扩大到最高 100,000 km²，但仍禁止把全国建筑和道路作为一次全量查询。高德 POI 可补充中国境内关键设施位置，不能替代建筑、道路存量统计或道路实时通行状态。
