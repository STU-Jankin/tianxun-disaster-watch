...
- 事件详情可按当前 AOI 查询 WorldPop 指定年份人口模型，并通过 OpenStreetMap Overpass 筛查已映射建筑、道路和医疗/应急/避难/教育/电力/供排水设施；AOI 与设施点会叠加到地图，结果按事件版本和 AOI 指纹持久化缓存。道路桥隧查询也写入共享 OSM 查询缓存，服务重启后仍可复用，并在上游短时失败时明确标记为过期缓存降级。
